...
The package comes with a seed that can populate the table with a whole bunch of sample posts. There are some configuration options for the seed in the config file. To run it:
    ...
    php artisan db:seed --class="Rowland\LaravelBlog\PostTableFakeSeeder"
...
Finally, update the IoC Container to inject an instance of your model into the `Rowland\LaravelBlog\PostsController`,
instead of the package's model, e.g. in `app/start/global.php`

```php
App::bind('Rowland\LaravelBlog\PostsController', function() {
    return new Rowland\LaravelBlog\PostsController(new Post);
});
```
...
your new model e.g. `Post` instead of `Rowland\LaravelBlog\Post`, and add the field to the `edit_fields` section to allow
...
class Post extends Rowland\LaravelBlog\Post {
...
As mentioned above, update the IoC Container to inject an instance of your model into the `Rowland\LaravelBlog\PostsController`,
instead of the package's model, e.g. in `app/start/global.php`
