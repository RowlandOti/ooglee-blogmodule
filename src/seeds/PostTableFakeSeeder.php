...
class PostTableFakeSeeder extends \Seeder {
	...
	public function run()
	{
		...
		$numberToCreate = \Config::get('laravelblog::seed.number');
	...
	{
		$replace = \Config::get('laravelblog::seed.replace');
		if ($replace)
		{
	...
	{
		$youTubeVideoFreq = \Config::get('laravelblog::seed.you_tube.freq');
	...
	{
		$mainImageFreq = \Config::get('laravelblog::seed.images.main_image.freq');
	...
	{
		$imageOptions = \Config::get('laravelblog::images.main_image');
		if (!$imageOptions['show'])
		{
			return false;
		}
		$seedOptions = \Config::get('laravelblog::seed.images.main_image');
	...
	{
		$showLink =  \Config::get('laravelblog::link.show');
		if (!$showLink)
		{
			return false;
		}
		$linkFreq = \Config::get('laravelblog::seed.link.freq');
	...
	{
		$linkTexts = \Config::get('laravelblog::seed.link.texts');
		$this->post->link_text = $this->faker->randomElement($linkTexts);
	}

	protected function setLinkUrl()
	{
		$linkUrls = \Config::get('laravelblog::seed.link.urls');
