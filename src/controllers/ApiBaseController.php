    ...
    */
    ...
    public function indexByYearMonth($selectedYear, $selectedMonth)
    {
        ...
        $posts = $this->post->live()
            ...
            ->paginate(\Config::get('laravelblog::views.index_page.results_per_page'));

        // Get the archives data if the config says to show the archives on the index page
        if (\Config::get('laravelblog::views.index_page.show_archives'))
        {
            $archives = $this->post->archives();
        }

        return \View::make(\Config::get('laravelblog::views.index_page.view'), compact('posts', 'selectedYear', 'selectedMonth', 'archives'));
    }
    ...
    public function indexByRelationship($relationshipIdentifier)
    {
        ...
        return \View::make(\Config::get('laravelblog::views.index_page.view'), compact('posts', 'archives'));
    }
    ...
    public function view($slug)
    {
        ...
        $newer = $older = false;
        if (\Config::get('laravelblog::views.view_page.show_adjacent_items'))
        {
            $newer = $post->newer();
            $older = $post->older();
        }
        ...
        if (\Config::get('laravelblog::views.view_page.show_archives'))
        {
            $archives = $this->post->archives();
        }

        return \View::make(\Config::get('laravelblog::views.view_page.view'), compact('post', 'newer', 'older', 'archives'));
    ...
    public function rss()
    {
        ...
        $feed->channel(array(
            'title' => \Config::get('laravelblog::meta.rss_feed.title'),
            'description' => \Config::get('laravelblog::meta.rss_feed.description'),
            'link' => \URL::current(),
        ));
        $posts = $this->post->live()
        ...
        foreach ($posts as $post){
            $feed->item(array(
                ...
                'link' => \URL::action('Rowland\LaravelBlog\PostsController@view', array('slug' => $post->slug)),
            ));
        }
